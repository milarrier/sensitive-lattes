...
function pltSbam1(Ns)
    nN = length(Ns)
    varel = zeros(nN)
    varbs = zeros(nN)
    w = [10.0^t for t in range(-4.0,3.0,10000)]
    for i in 1:nN
        N = Ns[i]
        for k in 1:2N+1
            r0 = frSN1D(N,k,N+1,w)
            r1 = frSN1D(N,k,N,w)
            rN = frSN1D(N,k,1,w)
            varel[i] += integrate(w, (abs.(r0-r1)).^2)/π
            varbs[i] += integrate(w, (abs.(r0-rN)).^2)/π
        end
    end
    plot(Ns, varel/2; c=:steelblue)
    plot!(Ns, varbs/2; legend=false)
end
...
function pltFRSN1D(Ns::Vector{Int64})
    ...
    for N in Ns
        ...
        plot!(w, abs.(r);
              palette=palette(:Blues, length(Ns)),
              xlims=(1e-2,1e2),
              # ylims=(1e-5,1e1),
              yscale=:log10,
              xscale=:log10,
